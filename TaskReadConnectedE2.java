...
public class TaskReadConnectedE2 extends Thread {
	...
	TaskReadConnectedE2(LockfreeQueue<CoupleResult> ALire,HashMap<String,Couple> rempli,LockfreeQueue<Trio> UrlEtPage, AtomicInteger N,int NombreGeneration){
		this.ALire=ALire;
		this.rempli=rempli;
    		...
    		try {
    			...
    			synchronized (rempli) {
    				if (this.rempli.containsKey(_url)){
    					boolean b=this.rempli.get(_url).b;
    					if(!b){continue;}
    					else {
    					int gen=this.rempli.get(_url).i;
    					this.rempli.put(_url, new Couple (b,Math.min(generation, gen)));}
    				}
 			...
 			boolean pasunedate=((_url.startsWith("/wiki/")) && !(_url.equals("/wiki/Article_nominations"))&&!(_url.startsWith("/wiki/LEGO"))&&
        	  ...
        	  if(pasunedate){
            	...
            	if(!pasunedate){
            		synchronized(rempli){ rempli.put(_url, new Couple(false,-1));}
            		continue;
            	}
